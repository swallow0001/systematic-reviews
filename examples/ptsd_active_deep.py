...
from python.model.textmanager import TextManager
from python.model.embedding import Word2VecEmbedding
from python.model.lstm import LSTM_Model
from python.config import LOG_DIR, OUTPUT_DIR, DATA_DIR, GLOVE_DIR
from python.libact.labeler import InteractivePaperLabeler
...
def main(args):
    ...
    pool, pool_ideal = make_pool(
        data, labels, prelabeled=[1, 2, 3, 4, 5, 218, 260, 466, 532, 564])
...
#     model = SklearnProbaAdapter(sklearn_model(**kwargs_model))
    ...
    acc_pool = np.append(
        acc_pool, model._model.score([x[0] for x in pool.get_entries()],
                                     labels))
    #
    #     # make plot
