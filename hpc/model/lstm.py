...
class LSTM_Model():
    ...
    def _get_scores(self,pred,label,allowed_FN):
        ...
        best_tn, best_fp, best_fn, best_tp = self._get_scores_threshhold(pred,label,threshholds[0])  
        ## calculate scores for each threshhold
        for i, threshhold in enumerate(threshholds):
            tn, fp, fn, tp = self._get_scores_threshhold(pred,label,threshhold)
            if (fn <= allowed_FN) and (best_fn > allowed_FN) and (threshhold>0):
                ...
                if (fn <= allowed_FN) and (best_fp >fp) and (threshhold>0):
                    ...
                    best_fn=fn
                    best_tp=tp
        return (float(best_tn), float(best_fp), float(best_fn), float(best_tp), pred.tolist())
